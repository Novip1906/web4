...
export class ProductComponent {
    ...
    parseRate(rateStr) {
        const match = String(rateStr).replace(",", ".").match(/[\d.]+/);
    ...
    buildToasts(data) {
        return {
            rate: {
                ...
                text: `${data.title}: ${data.rate}.`,
                variant: "success",
            },
            promo: {
                title: "Акции и бонусы",
                text: `Минимальная сумма — ${data.minAmount}, срок — ${data.term}.`,
                variant: "warning",
            },
            info: {
                title: "Условия",
                text: `Валюта: ${data.currency}. Уровень риска: ${data.risk}.`,
                variant: "info",
            },
        };
    }
